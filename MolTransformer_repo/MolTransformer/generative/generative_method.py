...
class GenerateMethods(IndexConvert):
    ...
    def _compute_descriptor(self,smile_list):
        ...
        # --- START: New method to add to GenerateMethods class ---
    def latent_space_2_logits(self, latent_space_tensor):
        """
        Decodes a latent space tensor directly to the raw output logits
        from the model's decoder, without converting to strings.

        Args:
            latent_space_tensor (torch.Tensor): A tensor representing the latent space,
                                                expected shape (batch_size, 401, 30).

        Returns:
            torch.Tensor: The raw output tensor (logits) from the decoder,
                          likely shape (401, batch_size, vocab_size).
        """
        # Ensure input is a torch tensor and on the correct device
        if not isinstance(latent_space_tensor, torch.Tensor):
            raise TypeError("Input must be a torch.Tensor")

        memory_torch = latent_space_tensor.to(self.device)

        # --- Input Shape Check and Permutation ---
        # The decoder expects shape (seq_len, batch_size, embed_dim)
        # which seems to be (401, B, 30) based on _memory_2_representation
        if memory_torch.shape[1] == 401 and memory_torch.shape[2] == 30:
            # Input shape is likely (B, 401, 30), permute it
            memory = memory_torch.permute(1, 0, 2)
            print(f"Permuted input tensor from {memory_torch.shape} to {memory.shape} for decoder.")
        elif memory_torch.shape[0] == 401 and memory_torch.shape[2] == 30:
             # Input shape is already (401, B, 30)
             memory = memory_torch
             print(f"Using input tensor shape {memory.shape} directly for decoder.")
        else:
            raise ValueError(f"Unexpected input tensor shape: {memory_torch.shape}. "
                             f"Expected (batch_size, 401, 30) or (401, batch_size, 30).")

        # Ensure the model itself is on the correct device
        self.model.to(self.device)

        # --- Call the Decoder ---
        # No torch.no_grad() here, allow gradients to flow
        if self.gpu_mode: # Assuming gpu_mode means DataParallel or similar
            molecule_logits = self.model.decoder(memory)
        else:
            molecule_logits = self.model.decoder(memory)

        print(f"Decoder output logits shape: {molecule_logits.shape}") # Expected: (401, B, vocab_size)
        return molecule_logits
    # --- END: New method ---